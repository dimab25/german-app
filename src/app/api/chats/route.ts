...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "All chats from DB retrieved successfully",
      amount: chats.length,
      data: chats,
    });
  } catch (error) {
    return NextResponse.json(
      { success: false, message: "Failed to fetch all chats from DB" },
      { status: 500 }
    );
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { success: false, message: "Failed to create new chat" },
      { status: 400 }
    );
  }
}
